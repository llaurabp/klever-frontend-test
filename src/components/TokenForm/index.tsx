...
export default function TokenForm(props: ITokenFormProps) {
  ...
  function validateToken(tokenName: string) {
    ...
    props.type === FormType.AddToken && tokens.forEach((token: IToken) => {
      if (tokenName.toLocaleLowerCase() === token.token.toLocaleLowerCase()) {
        throw new Error('Já existe um token com esse nome.');
      }
    })

    props.type === FormType.EditToken && tokens.forEach((token: IToken) => {
      if (tokenName.toLocaleLowerCase() === token.token.toLocaleLowerCase() && tokenName !== editToken) {
        throw new Error('Já existe um token com esse nome.');
      }
    })
  };
    ...
    <Container>
      ...
      <ButtonsGroup type={props.type}>
        ...
        <Button
          ...
          color='white'
          content='Save'
          onClick={() => props.type === FormType.EditToken ? updateToken({ token, balance }) : createToken()}
        />
      </ButtonsGroup>
    </Container>
  )
